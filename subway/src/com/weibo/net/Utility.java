...
public class Utility
{
	...
	// è®¾ç½®httpå¤´,å¦‚æžœauthParamä¸�ä¸ºç©ºï¼Œåˆ™è¡¨ç¤ºå½“å‰�æœ‰tokenè®¤è¯�ä¿¡æ�¯éœ€è¦�åŠ å…¥åˆ°å¤´ä¸­
			...
			throws WeiboException
	{
		...
		request.setHeader("User-Agent", System.getProperties().getProperty(
				"http.agent")
				+ " WeiboAndroidSDK");
	}
	...
	public static HttpClient getNewHttpClient(Context context)
	{
		try
		{
			...
			{
				// èŽ·å�–å½“å‰�æ­£åœ¨ä½¿ç”¨çš„APNæŽ¥å…¥ç‚¹
		...
		for (int loc = 0; loc < httpParams.size(); loc++)
		{
			...
			try
			{
				buf.append(URLEncoder.encode(key, "UTF-8")).append("=").append(
						URLEncoder.encode(httpParams.getValue(key), "UTF-8"));
	...
	public static InputStream getXmlData(String url)
	{
		InputStream is = null;
		try
		{
